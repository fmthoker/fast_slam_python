...
class Particle(object):
    ...
    class LandmarkEKF(object):
        ...
        def __init__(self):
            self.observed = False
            self.mu = np.zeros((2,1))  # landmark position as vector of length 2
    ...
    def correction_step(self, sensor_measurements):
		...
		landmark.mu[0] = robot_pose[0]+ measurement.z_range * cos((measurement.z_bearing + robot_pose[2]))
		landmark.mu[1] =  robot_pose[1] +measurement.z_range * sin((measurement.z_bearing + robot_pose[2]))
                # get the Jacobian
                [h, H] = self.measurement_model(landmark)
                # TODO: initialize the EKF for this landmark
		landmark.sigma = np.dot(np.linalg.inv(H),np.dot(Q_t,np.matrix.transpose(np.linalg.inv(H))))
		...
		measurement_co_variance = np.dot(H, np.dot(landmark.sigma , np.matrix.transpose(H) ))+ Q_t

                # TODO: calculate the Kalman gain
		kalman_gain = np.dot(np.dot(landmark.sigma , np.matrix.transpose(H)) , np.linalg.inv(measurement_co_variance))
		...
		temp = np.dot(kalman_gain,error)

                # TODO: update the mean and covariance of the EKF for this landmark
		landmark.mu[0] = landmark.mu[0] + temp[0] 
		landmark.mu[1] = landmark.mu[1] + temp[1] 
		landmark.sigma = np.dot((np.identity(2)  - np.dot(kalman_gain,H)),landmark.sigma)

                # TODO: compute the likelihood of this observation, multiply with the former weight
                # to account for observing several features in one time step
        	self.weight = self.weight * (1 / sqrt(2 * pi * (np.linalg.det(measurement_co_variance) ))) * exp( np.dot((-1/2)*(np.matrix.transpose(error)) , np.dot(np.linalg.inv(measurement_co_variance),error)))
